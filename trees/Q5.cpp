...
int countNodes(NodeTree *root, int &i) {
	if(root != NULL) {
		i++;
...
int sumNodes(NodeTree *root, double &sum) {
	if(root != NULL) {
...
int main() {
	...
	i = countNodes(root, i);
	sum = sumNodes(root, sum);
	
	double averageNodes = sum / i;
	
	cout << "media dos nos: " << averageNodes << endl;
